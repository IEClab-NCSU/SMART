...
L = length(df$Anon.Student.Id)
success = vector(mode="numeric", length=L)
success[df$Success=="correct"]=1
...
#Dropped a tern in the above equation for simpler model
...
# afm.model.st <- lmer(Success ~ KC + KC:Opportunity+(1|Anon.Student.Id),data=df,family=binomial())
