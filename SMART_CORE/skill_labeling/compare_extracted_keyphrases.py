...
def prepare_file_system():
    output_path = 'keyphrase_comparison/'
    if os.path.exists(output_path):
        shutil.rmtree(output_path)
    os.mkdir(output_path)


def find_best_worst_f1(predicted_kp_list, gold_kp_list):
    best_f1 = 0
    worst_f1 = np.inf
    best_f1_index = 0
    worst_f1_index = 0

    for index in range(0, len(gold_kp_list)):
        predicted_keyphrases = [stem(text) for text in predicted_kp_list[index]]
        predicted_keyphrases = [remove_stopwords(text) for text in predicted_keyphrases]
        gold_keyphrases = [stem(text) for text in gold_kp_list[index]]
        gold_keyphrases = [remove_stopwords(text) for text in gold_keyphrases]

        num_pred_tokens, num_tgt_tokens, num_common_tokens = calc_public_stats(predicted_keyphrases, gold_keyphrases)

        if num_pred_tokens > 0:
            part_prec = num_common_tokens / num_pred_tokens
        else:
            part_prec = 0.0

        if num_tgt_tokens > 0:
            part_recall = num_common_tokens / num_tgt_tokens
        else:
            part_recall = 0.0

        if part_prec + part_recall > 0:
            partial_f1 = 2 * (part_prec * part_recall) / (part_prec + part_recall)
        else:
            partial_f1 = 0.0

        if partial_f1 > best_f1:
            best_f1 = partial_f1
            best_f1_index = index
        if partial_f1 < worst_f1:
            worst_f1 = partial_f1
            worst_f1_index = index

    return best_f1_index, worst_f1_index
...
def main():
    ...
    for dataset in datasets:
        ...
        keybert_results_path = f'OneDrive/SMART/Jesse_2022/skill_labeling/AKE_study_2_28_2022/output/ake_results/{dataset}_parsing_sbert-mean-pooling_cos-doc-cand_top-n_{dataset_n}_ae_off_trial_1_output.csv'
        ...
        rake_results_path = f'OneDrive/SMART/Jesse_2022/skill_labeling/AKE_study_2_28_2022/output/ake_results/{dataset}_RAKE_{dataset_n}_trial_1_output.csv'
        rake_results_path = os.path.join(str(Path.home()), rake_results_path)
        yake_results_path = f'OneDrive/SMART/Jesse_2022/skill_labeling/AKE_study_2_28_2022/output/ake_results/{dataset}_YAKE_{dataset_n}_trial_1_output.csv'
        yake_results_path = os.path.join(str(Path.home()), yake_results_path)
        textrank_results_path = f'OneDrive/SMART/Jesse_2022/skill_labeling/AKE_study_2_28_2022/output/ake_results/{dataset}_TextRank_{dataset_n}_trial_1_output.csv'
        textrank_results_path = os.path.join(str(Path.home()), textrank_results_path)
        singlerank_results_path = f'OneDrive/SMART/Jesse_2022/skill_labeling/AKE_study_2_28_2022/output/ake_results/{dataset}_SingleRank_{dataset_n}_trial_1_output.csv'
        singlerank_results_path = os.path.join(str(Path.home()), singlerank_results_path)
        topicrank_results_path = f'OneDrive/SMART/Jesse_2022/skill_labeling/AKE_study_2_28_2022/output/ake_results/{dataset}_TopicRank_{dataset_n}_trial_1_output.csv'
        topicrank_results_path = os.path.join(str(Path.home()), topicrank_results_path)
        multipartiterank_results_path = f'OneDrive/SMART/Jesse_2022/skill_labeling/AKE_study_2_28_2022/output/ake_results/{dataset}_MultipartiteRank_{dataset_n}_trial_1_output.csv'
