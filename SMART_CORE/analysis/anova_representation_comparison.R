...
filename <- 'representation_comparison_model_fit_data_long.csv'
data <- read.csv(file = filename, header = TRUE, sep = ",")

# train model
print(paste('Training Model for Regression Test and ANOVA: ', Sys.time()))
data$representation <- factor(data$representation, levels=c("tf-idf", "bert", "s-bert"), ordered=FALSE)
...
data$num_clusters <- as.numeric(as.vector(data$num_clusters))
data$aic <- as.numeric(as.vector(data$aic))

# run regression test and ANOVA
model <- lmer(aic ~ representation + focal_strategy + num_clusters + (1|course), data=data)
summary(model)
anova(model)
